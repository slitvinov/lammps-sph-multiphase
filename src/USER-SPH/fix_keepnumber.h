...
FixStyle(keepnumber,FixKeepNumber)

#else

#ifndef LMP_FIX_KEEPNUMBER_H
#define LMP_FIX_KEEPNUMBER_H

#include "fix.h"

namespace LAMMPS_NS {

class FixKeepNumber : public Fix {
 public:
  FixKeepNumber(class LAMMPS *, int, char **);
  ~FixKeepNumber();
  int setmask();
  void init();
  void pre_exchange();
