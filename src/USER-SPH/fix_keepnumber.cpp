...
FixKeepNumber::FixKeepNumber(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (narg < 5) error->all(FLERR,"Illegal fix keepnumber command");
  ...
  if (iregion == -1)
    error->all(FLERR,"Region ID for fix keepnumber does not exist");
  if (seed <= 0) error->all(FLERR,"Illegal fix keepnumber command");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"molecule") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix keepnumber command");
      if (strcmp(arg[iarg+1],"no") == 0) molflag = 0;
      else if (strcmp(arg[iarg+1],"yes") == 0) molflag = 1;
      else error->all(FLERR,"Illegal fix keepnumber command");
      iarg += 2;
    } else error->all(FLERR,"Illegal fix keepnumber command");
  }

  // we keep this number of atom in the group
  nkeep = group->count(igroup);
...
void FixKeepNumber::init()
{
  ...
  if (molflag == 0 && atom->molecule_flag) {
    ...
    if (flagall && comm->me == 0)
      error->warning(FLERR,
                     "Fix keepnumber may delete atom with non-zero molecule ID");
  }

  if (molflag && atom->molecule_flag == 0)
      error->all(FLERR,
                 "Fix keepnumber molecule requires atom attribute molecule");
}
...
void FixKeepNumber::pre_exchange()
{
  ...
  if (atom->nlocal > nmax) {
    ...
    memory->destroy(mark);
    nmax = atom->nmax;
    memory->create(list,nmax,"keepnumber:list");
    memory->create(mark,nmax,"keepnumber:mark");
  }
...
double FixKeepNumber::compute_scalar()
{
  return 1.0*ndeleted;
}
...
double FixKeepNumber::memory_usage()
{
